...
test()
{
	...
	case $problem in
	    ...
	    *)
		...
		command="swipl -s $1.pl -g \"command_line.\" test/$2/domain-$2.pddl $problem 2>> error.log"
		echo $command
		output=`swipl -s $1.pl -g "command_line." test/$2/domain-$2.pddl $problem`   2>> error.log  
		if [ "$output" = "" ]
		then
		    break
		else	
