...
test()
{
	...
	case $problem in
	    ...
	    *)
		...
		command="/var/lib/myfrdcsa/codebases/cats/game-engines/prolog-game-engine/attempts/5/flux/eclipse -f $1.pl -e \"command_line.\" test/$2/domain-$2.pddl $problem 2>> error.log"
		echo $command
		output=`/var/lib/myfrdcsa/codebases/cats/game-engines/prolog-game-engine/attempts/5/flux/eclipse -f $1.pl -e "command_line." test/$2/domain-$2.pddl $problem`   2>> error.log  
		if [ "$output" = "" ]
		then
		    break
		else	
