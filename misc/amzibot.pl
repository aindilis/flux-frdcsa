...
%% consistent(Z) :- here(Z), duplicate_free(Z).
...
main :- setup, init(Z), request_plan(Z, S), see([1]), translatePlanFromCanonical(S,TS), see([2]), nl, writeln([plan,TS]).
...
request_plan(S, Z, [goto(48368685518133,551826475005)]) :- true.
